...
export default function CaixaPage() {
  ...
  useEffect(() => {
    const unsubRows = onSnapshot(collection(db, 'cashRegisters'), (snap) => {
      const items = snap.docs.map((d) => ({ id: d.id, ...(d.data() as Omit<CashRegister, 'id'>) }));
      items.sort((a, b) => (b.openedAt || '').localeCompare(a.openedAt || ''));
      setCashRows(items);
    });
    if (!profile) return () => unsubRows();
    const unsubOpen = onSnapshot(query(collection(db, 'cashRegisters'), where('status', '==', 'aberto'), where('operatorId', '==', profile.id)), (snap) => {
  ...
  const withdrawalTotal = useMemo(() => openCash?.withdrawals?.reduce((sum, item) => sum + item.amount, 0) || 0, [openCash]);
  const partialBalance = openCash ? openCash.openingAmount + openCash.revenueByTickets + openCash.revenueByMonthly - withdrawalTotal : 0;

  async function handleOpen(event: FormEvent) {
    event.preventDefault();
    if (!profile || openCash) return;

    const existingOpenCash = await getDocs(query(collection(db, 'cashRegisters'), where('status', '==', 'aberto'), where('operatorId', '==', profile.id)));
  ...
  async function handleWithdraw(event: FormEvent) {
    ...
    await updateDoc(doc(db, 'cashRegisters', openCash.id), {
      withdrawals: [...(openCash.withdrawals || []), { amount: toInputNumber(withdrawAmount), reason: withdrawReason, createdAt: new Date().toISOString() }],
    });
    setWithdrawAmount('');
    setWithdrawReason('');
  ...
  async function handleCloseCash() {
    ...
    await updateDoc(doc(db, 'cashRegisters', openCash.id), { status: 'fechado', closedAt });
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          !openCash ? (
            <button className="primary-button" onClick={() => setShowOpenForm((v) => !v)}><Plus size={16} />Abrir Caixa</button>
          ) : (
            <div className="flex flex-wrap gap-3">
              <button className="secondary-button" onClick={() => setShowWithdrawForm((v) => !v)}><BanknoteArrowUp size={16} />Sangria</button>
              <button className="primary-button" onClick={handleCloseCash}><Wallet size={16} />Fechar Caixa</button>
            </div>
          )
        }
      />
      ...
      {showOpenForm && !openCash ? (
        <div className="panel-card mb-6 p-6">
          ...
          <form className="mt-4 flex flex-wrap gap-3" onSubmit={handleOpen}>
            <input className="app-input max-w-sm" placeholder="Valor Inicial (R$)" value={openingAmount} onChange={(e) => setOpeningAmount(e.target.value)} />
            <button className="primary-button">Abrir Caixa</button>
            <button className="secondary-button" type="button" onClick={() => setShowOpenForm(false)}>Cancelar</button>
          </form>
        </div>
      ) : null}
      ...
      {openCash ? (
        <div className="mb-6 panel-card p-6">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            <div className="rounded-3xl bg-slate-50 p-4"><p className="text-sm text-slate-500">Operador</p><p className="mt-2 font-semibold text-slate-900">{openCash.operatorName}</p></div>
            <div className="rounded-3xl bg-slate-50 p-4"><p className="text-sm text-slate-500">Abertura</p><p className="mt-2 font-semibold text-slate-900">{shortDateTime(openCash.openedAt)}</p></div>
            <div className="rounded-3xl bg-slate-50 p-4"><p className="text-sm text-slate-500">Valor Inicial</p><p className="mt-2 font-semibold text-slate-900">{money(openCash.openingAmount)}</p></div>
            <div className="rounded-3xl bg-slate-50 p-4"><p className="text-sm text-slate-500">Faturamento</p><p className="mt-2 font-semibold text-slate-900">{money(openCash.revenueByTickets + openCash.revenueByMonthly)}</p></div>
            <div className="rounded-3xl bg-slate-50 p-4"><p className="text-sm text-slate-500">Saldo Parcial</p><p className="mt-2 font-semibold text-slate-900">{money(partialBalance)}</p></div>
          </div>
      ...
      <div className="panel-card p-6">
        ...
        <div className="table-shell mt-4">
          <table>
            ...
            <tbody>
              {cashRows.length ? cashRows.map((row) => {
                const sangrias = row.withdrawals?.reduce((sum, item) => sum + item.amount, 0) || 0;
                const finalValue = row.openingAmount + row.revenueByTickets + row.revenueByMonthly - sangrias;
                return (
                  <tr key={row.id}>
